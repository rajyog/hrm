        ...
        function getuser(uid)
         {
            ...
            $.ajax({
                url: '{{route('event.getuser')}}',
                type: 'POST',
                data: {
                    "user_id": uid, "_token": "{{ csrf_token() }}",
                },
                success: function (data) {

                    $('#department_id').empty();
                    $('#department_id').append('<option value="">{{__('Select user')}}</option>');

                    $('#department_id').append('<option value="0"> {{__('All user')}} </option>');
